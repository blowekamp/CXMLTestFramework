...
protected:
  ...
  {
    std::cout << fileutil::PathName(TF_DATA_BASELINE).Append("somefile.txt").GetPathName() << std::endl;
...
int tfRegressionTestOutputTest2( int argc, char *argv[] ) 
{
  ...
  if ( 3 == ret  ) 
    ...
    {
    std::cerr << "Expected regression test to return 5 measurements not matching" << std::endl;
    return EXIT_FAILURE;
    }
}
